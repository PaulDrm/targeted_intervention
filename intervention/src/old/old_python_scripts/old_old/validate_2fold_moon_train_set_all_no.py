...
def main(): 
    ...
    for i in range(args.num_fold):
        ...
        top_heads, probes = get_top_heads(train_set_idxs, val_set_idxs, separated_activations, separated_labels, num_layers, num_heads, args.seed, args.num_heads, args.use_random_dir, specific_heads=[(30,24)]) #(13,0)]) #(14, 1)]) #[(31,14)])
        ...
        curr_fold_results.to_json(f"../intervention_results/results_test_openchat_intervention_no_train_{int(args.alpha)}_{args.num_heads}_fold_{i}.json", orient='records', indent=4)
        ...
        print(f"Train data: Precision: {precision}, Recall: {recall} for fold {i} and alpha {args.alpha} and number of heads {args.num_heads}")
        ...
        with open('../intervention_results/overall_results.txt', 'a') as f:
            ...
            print(f"Train data: Precision: {precision}, Recall: {recall} for fold {i} and alpha {args.alpha} and number of heads {args.num_heads}", file=f)
        ...
        curr_fold_results.to_json(f"../intervention_results/results_test_openchat_intervention_no_valid_{int(args.alpha)}_{args.num_heads}_fold_{i}.json", orient='records', indent=4)
        ...
        print(f"Valid data: Precision: {precision}, Recall: {recall} for fold {i} and alpha {args.alpha} and number of heads {args.num_heads}")
        print(curr_fold_results.predict.value_counts())
        with open('../intervention_results/overall_results.txt', 'a') as f:
            # Redirect the print output to the file
            print(f"Valid data: Precision: {precision}, Recall: {recall} for fold {i} and alpha {args.alpha} and number of heads {args.num_heads}", file=f)
            print(curr_fold_results.predict.value_counts(),file = f)
