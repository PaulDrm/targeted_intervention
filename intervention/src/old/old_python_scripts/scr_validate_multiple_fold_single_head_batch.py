...
def main(): 
    ...
    for i in range(args.num_fold):
        ...
        if args.num_heads == 1:
            curr_fold_results.to_json(f"{args.output_path}/results_intervention_{int(args.alpha)}_head_{head_string}_fold_{fold}.json", orient='records', indent=4)
        else:
            curr_fold_results.to_json(f"{args.output_path}/results_intervention_{int(args.alpha)}_number_heads_{len(top_heads)}_fold_{fold}.json", orient='records', indent=4)
