...
CONFIG=$(jq '.' experiments/ai_coordination/config_example_373_bulk_com.json)
...
for alpha in 55 75; do
    echo "alpha: $alpha"
    python scr_validate_sweep_heads_probing.py --alpha $alpha --val_ratio $val_ratio --model_name $model_name  --input_path $input_path --output_path $output_path --dataset $dataset_name --prompt_type $prompt_type --use_center_of_mass $use_center_of_mass 
    echo
    echo
    done
done
