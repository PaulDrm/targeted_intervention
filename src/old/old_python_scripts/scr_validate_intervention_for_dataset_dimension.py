...
import sys
sys.path.append('../')
from src.old.utils import alt_tqa_evaluate, flattened_idx_to_layer_head, layer_head_to_flattened_idx, get_interventions_dict, get_top_heads, get_separated_activations, get_com_directions, load_model, prepare_prompt,extract_final_answer_dataset
...
def main(): 
    ...
    for i in range(args.num_fold):
        ...
        for dim in range(128):
            ...
            def lt_modulated_vector_add(head_output, layer_name, start_edit_location='lt'): 
                ...
                for head, direction, proj_val_std in interventions[layer_name]:
                    if start_edit_location == 'lt': 
                        #print(dim)
                        #direction_to_add = args.alpha * proj_val_std * direction_to_add
                        #head_output[:, -1, head, :] += direction_to_add
                        #print(direction.shape)
                        # Create a zero tensor with the same shape as head_output[:, -1, head, :]
                        direction_to_add = torch.zeros_like(head_output[:, -1, head, :]).flatten()
                        #print(direction_to_add.shape)
                        #direction_to_add = torch.tensor(direction).to(head_output.device.index)
                        #print(direction_to_add.shape)
                        # Set the non-zero value at the specified index
                        direction_to_add[dim] = direction[dim] * args.alpha * proj_val_std
                        print(direction_to_add)
                        # Add the modified direction to the head_output
                        head_output[:, -1, head, :] += direction_to_add
            ...
            curr_fold_results.to_json(f"{args.output_path}/results_{int(args.alpha)}_{args.layer}_{args.head}_{str(args.use_center_of_mass)}_{dim}.json", orient='records', indent=4)
            ...
            if args.prompt_type != "open_ended":
                ...
                with open(f'{args.output_path}/overall_results.txt', 'a') as f:
                    ...
                    print(f"For dataset fold {i} and alpha {args.alpha} and head {args.layer} {args.head} and dim {dim}", file=f)
