...
def main(): 
    ...
    parser.add_argument('--prompt_type', type=str, help='prompt type', default="ab_cot")
    ...
    for i in range(args.num_fold):
        ...
        #print(size)
        train_set_idxs = train_idxs#rng.choice(train_idxs, size=size, replace=False)
        ...
        for row in tqdm(test_set.iterrows()):
            ...
            **generation_args, 
            ...
            if args.dataset_name != "requirements_data" and args.prompt_type != "open_ended":
                ...
                results.append({id_column: row[1][id_column], "prompt": prompt, "output": output, "final_answer": final_answer, "predict": predict, "score": score, "gt": gt}),
        ...
        curr_fold_results.to_json(f"{args.output_path}/results_test_{int(args.alpha)}_{args.layer}_{args.head}_fold_{i}.json", orient='records', indent=4)
        ...
        with open(f'{args.output_path}/overall_results.txt', 'a') as f:
            ...
            print(f"For dataset fold {i} and alpha {args.alpha} and head {args.layer} {args.head}", file=f)
