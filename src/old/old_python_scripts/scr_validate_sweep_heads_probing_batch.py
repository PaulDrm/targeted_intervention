...
import sys
sys.path.append('../')
from src.old.utils import alt_tqa_evaluate, flattened_idx_to_layer_head, layer_head_to_flattened_idx, get_interventions_dict, get_top_heads, get_separated_activations, get_com_directions, load_model

sys.path.append('../app/')
from intervention.reasoning import eval_intervention, parse_output, extract_final_answer, eval_intervention_batch
...
def main(): 
    ...
    if not args.use_center_of_mass:
        ...
        probes = []
    for layer in range(9,num_layers):
        ...
        for h in range(num_heads):
            ...
            if args.dataset_name == "requirements_data":
                #false_positives = train_set[(train_set['correct'] == 0) & (train_set['final_answer']== True)].iloc[0:20]
                correct_examples = train_set[(train_set['correct'] == 1)].iloc[0:8]

                # Filter the DataFrame based on the conditions
                filtered_df = train_set[(train_set['correct'] == 0) & (train_set['final_answer'] == True)]

                # Get the value counts of req_id
                value_counts = filtered_df['req_id'].value_counts()

                # Perform a cumulative sum
                cumulative_sum = value_counts.cumsum()
                n=12
                # Find the keys until the cumulative sum reaches or exceeds 18
                keys_until_sum_reaches_n = cumulative_sum[cumulative_sum <= n].index

                # Print the result
                #print(keys_until_sum_reaches_n)

                false_positives = train_set[train_set['req_id'].isin(keys_until_sum_reaches_n)]
                data_set = pd.concat([false_positives, correct_examples])
            ...
            batch_size = 4
            for start_idx in tqdm(range(0, len(data_set), batch_size)):
                end_idx = min(start_idx + batch_size, len(data_set))
                batch = data_set.iloc[start_idx:end_idx]
